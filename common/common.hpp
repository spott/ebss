...
// throw std::out_of_range("the two vectors don't have the same local
// structure");
...
void map_function( Vec& vector, Func f, Vec& out )
{
    ...
    if ( ostart != vstart || oend != vend )
        throw std::out_of_range(
            "the two vectors don't have the same local structure" );
...
Mat petsc_binary_read<Mat>( const std::string& filename, MPI_Comm comm )
{
    ...
    if ( rank == 0 ) std::cerr << "done " << block_size << std::endl;
    return v;
}
...
template <typename T>
std::vector<T> merge_vectors( const std::vector<T>& v1,
...
// template <typename T>
// void export_vector_binary(const std::string &filename, const std::vector<T>&
// out)
//{
...
// else
//{
// std::cerr << "error opening file... does the folder exist?: " << filename <<
// std::endl;
...
void export_vector_binary( const std::string&    filename,
...
template <typename T>
std::vector<T>& import_binary_to_vector( const std::string& filename,
                                         std::vector<T>&    vec )
{
    std::ios::pos_type size;
    std::ifstream      file;
...
template <typename T>
void export_vector_ascii( const std::string&    filename,
                          const std::vector<T>& out )
{
    std::ios::pos_type size;
    std::ofstream      file;
...
// template <>
// Vec vector_to_Vec<PetscScalar>(std::vector< PetscScalar > vector, MPI_Comm
// comm)
//{
// int size;
...
std::vector<PetscScalar> Vec_to_vector( Vec v )
{
    ...
    MPI_Comm_rank( comm, &rank );
    if ( rank == 0 )
        std::cerr << "entering Vec_to_vector, size = " << size << std::endl;
    Vec        seq;
    VecScatter sc;
    VecCreate( comm, &seq );
    // if (rank == 0)
...
Mat populate_matrix( const Parameters& params, Test test, FindValue find_value,
                     const int mat_size_m, const int mat_size_n,
                     // const unsigned int diagonal_storage,
                     // const unsigned int offdiag_storage,
                     const bool symmetric = true, std::ostream& os = std::cout )
{
    assert( mat_size_m > 0 );
    assert( mat_size_n > 0 );
    ...
    MatSetSizes( H, PETSC_DECIDE, PETSC_DECIDE, mat_size_m, mat_size_n );
    ...
    for ( PetscInt i = rowstart; i < rowend; i++ ) {
        for ( PetscInt j = ( symmetric ? i : 0u ); j < mat_size_n; j++ ) {
            if ( test( i, j ) ) {
                ...
                if ( symmetric ) MatSetValue( H, j, i, value, INSERT_VALUES );
            }
        }
