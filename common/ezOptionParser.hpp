...
v0.1.2 20111126 rsz Allow flag names start with alphanumeric (previously, flag
had to start with alpha).
v0.1.3 20120108 rsz Created work-around for unique id generation with
IDGenerator that avoids retarded c++ translation unit linker errors with
single-header static variables. Forced inline on all methods to please retard
compiler and avoid multiple def errors.
...
For example, -d < --dimension < --dmn, and also lower come before upper. The
default STL std::string compare doesn't do that.
*/
...
/* ################################################################### */
// Variant that uses deep copies and references instead of pointers (less
// efficient).
static void SplitDelim( const std::string& s, const char token,
                        std::vector<std::string>& result )
{
...
/* ################################################################### */
static void StringsToInts( std::vector<std::string>& strings,
                           std::vector<int>&         out )
{
...
/* ################################################################### */
static void StringsToLongs( std::vector<std::string>& strings,
                            std::vector<long>&        out )
{
    for ( int i = 0; i < (long int)strings.size(); ++i ) {
...
/* ################################################################### */
static void StringsToULongs( std::vector<std::string>&   strings,
...
/* ################################################################### */
static void StringsToFloats( std::vector<std::string>& strings,
...
/* ################################################################### */
static void StringsToDoubles( std::vector<std::string>& strings,
...
/* ################################################################### */
static void StringsToStrings( std::vector<std::string*>* strings,
...
/* ################################################################### */
// Create unique ids with static and still allow single header that avoids
...
If comparing with a range, the values list must contain one or two values. One
value is required when comparing with <, <=, >, >=. Use two values when
requiring a test such as <x<, <=x<, <x<=, <=x<=.
A regcomp/regexec based class could be created in the future if a need arises.
*/
class ezOptionValidator
{
  public:
    inline ezOptionValidator( const char* _type, const char* _op = 0,
                              const char* list = 0, bool _insensitive = false );
    inline ezOptionValidator( char _type );
    inline ezOptionValidator( char _type, char _op, const char* list,
                              int _size );
    inline ezOptionValidator( char _type, char _op, const unsigned char* list,
                              int _size );
    inline ezOptionValidator( char _type, char _op, const short* list,
                              int _size );
    inline ezOptionValidator( char _type, char _op, const unsigned short* list,
                              int _size );
    inline ezOptionValidator( char _type, char _op, const int* list,
                              int _size );
    inline ezOptionValidator( char _type, char _op, const unsigned int* list,
                              int _size );
    inline ezOptionValidator( char _type, char _op, const long long* list,
                              int _size );
    inline ezOptionValidator( char _type, char _op,
                              const unsigned long long* list, int _size = 0 );
    inline ezOptionValidator( char _type, char _op, const float* list,
                              int _size );
    inline ezOptionValidator( char _type, char _op, const double* list,
                              int _size );
    inline ezOptionValidator( char _type, char _op, const char** list,
                              int _size, bool _insensitive );
...
/* ------------------------------------------------------------------- */
ezOptionValidator::~ezOptionValidator() { reset(); };
/* ------------------------------------------------------------------- */
...
/* ------------------------------------------------------------------- */
ezOptionValidator::ezOptionValidator( char _type, char _op, const char* list,
                                      int _size )
    : s1( 0 ), op( _op ), quiet( 0 ), type( _type ), size( _size ),
      insensitive( 0 )
{
    id = ezOptionParserIDGenerator::instance().next();
    s1 = new char[size];
...
/* ------------------------------------------------------------------- */
ezOptionValidator::ezOptionValidator( char _type, char _op,
                                      const unsigned char* list, int _size )
    : u1( 0 ), op( _op ), quiet( 0 ), type( _type ), size( _size ),
      insensitive( 0 )
{
    id = ezOptionParserIDGenerator::instance().next();
...
/* ------------------------------------------------------------------- */
ezOptionValidator::ezOptionValidator( char _type, char _op, const short* list,
                                      int _size )
    : s2( 0 ), op( _op ), quiet( 0 ), type( _type ), size( _size ),
      insensitive( 0 )
{
    id = ezOptionParserIDGenerator::instance().next();
    s2 = new short[size];
...
/* ------------------------------------------------------------------- */
ezOptionValidator::ezOptionValidator( char _type, char _op,
                                      const unsigned short* list, int _size )
    : u2( 0 ), op( _op ), quiet( 0 ), type( _type ), size( _size ),
      insensitive( 0 )
{
    id = ezOptionParserIDGenerator::instance().next();
...
/* ------------------------------------------------------------------- */
ezOptionValidator::ezOptionValidator( char _type, char _op, const int* list,
                                      int _size )
    : s4( 0 ), op( _op ), quiet( 0 ), type( _type ), size( _size ),
      insensitive( 0 )
{
    id = ezOptionParserIDGenerator::instance().next();
    s4 = new int[size];
...
/* ------------------------------------------------------------------- */
ezOptionValidator::ezOptionValidator( char _type, char _op,
                                      const unsigned int* list, int _size )
    : u4( 0 ), op( _op ), quiet( 0 ), type( _type ), size( _size ),
      insensitive( 0 )
{
    id = ezOptionParserIDGenerator::instance().next();
...
/* ------------------------------------------------------------------- */
ezOptionValidator::ezOptionValidator( char _type, char _op,
                                      const long long* list, int _size )
    : s8( 0 ), op( _op ), quiet( 0 ), type( _type ), size( _size ),
      insensitive( 0 )
{
    id = ezOptionParserIDGenerator::instance().next();
...
ezOptionValidator::ezOptionValidator( char _type, char _op,
                                      ...
                                      int                       _size )
    : u8( 0 ), op( _op ), quiet( 0 ), type( _type ), size( _size ),
      insensitive( 0 )
{
    id = ezOptionParserIDGenerator::instance().next();
...
/* ------------------------------------------------------------------- */
ezOptionValidator::ezOptionValidator( char _type, char _op, const float* list,
                                      int _size )
    : f( 0 ), op( _op ), quiet( 0 ), type( _type ), size( _size ),
      insensitive( 0 )
{
    id = ezOptionParserIDGenerator::instance().next();
    f  = new float[size];
...
/* ------------------------------------------------------------------- */
ezOptionValidator::ezOptionValidator( char _type, char _op, const double* list,
                                      int _size )
    : d( 0 ), op( _op ), quiet( 0 ), type( _type ), size( _size ),
      insensitive( 0 )
{
    id = ezOptionParserIDGenerator::instance().next();
    d  = new double[size];
...
ezOptionValidator::ezOptionValidator( char _type, char _op, const char** list,
                                      int _size, bool _insensitive )
    : t( 0 ), op( _op ), quiet( 0 ), type( _type ), size( _size ),
...
*/
ezOptionValidator::ezOptionValidator( const char* _type, const char* _op,
                                      const char* _list, bool _insensitive )
    : t( 0 ), quiet( 0 ), type( 0 ), size( 0 ), insensitive( _insensitive )
{
    ...
    int i = 0;
    for ( it = split.begin(); it != split.end(); ++it ) strings[i++] = *it;

    if ( insensitive )
        for ( i = 0; i < size; ++i ) ToLowerASCII( *strings[i] );

#define FreeStrings()                                                          \
    {                                                                          \
        for ( i = 0; i < size; ++i ) delete strings[i];                        \
        delete[] strings;                                                      \
    }
...
#define CHECKRANGE( E, T )                                                     \
    {                                                                          \
        ...
        if ( E##value < E##min ) {                                             \
            if ( !quiet )                                                      \
                std::cerr << "ERROR: Invalid value " << E##value               \
                          << " is less than datatype min " << E##min << ".\n"; \
            return false;                                                      \
        }                                                                      \
                                                                               \
        long long E##max =                                                     \
        ...
        if ( E##value > E##max ) {                                             \
            if ( !quiet )                                                      \
                std::cerr << "ERROR: Invalid value " << E##value               \
                          << " is greater than datatype max " << E##max        \
                          << ".\n";                                            \
            return false;                                                      \
        }                                                                      \
    }
        ...
        switch ( type ) {
            ...
            case S8: {
                if ( ( valueAsString->at( 0 ) == '-' ) &&
                     ...
                     ( valueAsString->compare( 1, 19, "9223372036854775808" ) >
                       0 ) ) {
                    if ( !quiet )
                        std::cerr << "ERROR: Invalid value " << *valueAsString
                                  << " is less than datatype min "
                                     "-9223372036854775808.\n";
                    return false;
                }
            ...
            case F: {
                ...
                if ( dvalue < dmin ) {
                    if ( !quiet ) {
                        fprintf( stderr,
                                 "ERROR: Invalid value %g is less than "
                                 "datatype min %g.\n",
                                 dvalue, dmin );
                    }
                    return false;
                ...
                if ( dvalue > dmax ) {
                    if ( !quiet )
                        std::cerr << "ERROR: Invalid value " << dvalue
                                  << " is greater than datatype max " << dmax
                                  << ".\n";
                    return false;
                }
            } break;
            case D: {
                long double ldmax = static_cast<long double>(
                ...
                if ( ldvalue < ldmin ) {
                    if ( !quiet )
                        std::cerr << "ERROR: Invalid value " << ldvalue
                                  << " is less than datatype min " << ldmin
                                  << ".\n";
                    return false;
                }
                ...
                if ( ldvalue > ldmax ) {
                    if ( !quiet )
                        std::cerr << "ERROR: Invalid value " << ldvalue
                                  << " is greater than datatype max " << ldmax
                                  << ".\n";
                    return false;
                }
            } break;
        ...
        switch ( op ) {                                                        \
            case LT:                                                           \
                if ( size > 0 ) {                                              \
                    ...
                    std::cerr << "ERROR: No value given to validate if " << v  \
                              << " < X.\n";                                    \
                    return false;                                              \
                }                                                              \
                break;                                                         \
            ...
            case GTLT:                                                         \
                if ( size > 1 ) {                                              \
                    ...
                    std::cerr << "ERROR: Missing values to validate if X1 < "  \
                              << v << " < X2.\n";                              \
                    return false;                                              \
                }                                                              \
                break;                                                         \
            case GELT:                                                         \
                if ( size > 1 ) {                                              \
                    ...
                    std::cerr                                                  \
                        << "ERROR: Missing values to validate if X1 <= " << v  \
                        << " < X2.\n";                                         \
                    return false;                                              \
                }                                                              \
                break;                                                         \
...
class OptionGroup
{
  public:
    ...
    {
        int i;
        for ( i = 0; i < (long int)flags.size(); ++i ) delete flags[i];
...
class ezOptionParser
{
  public:
    ...
    inline void add( const char* defaults, bool required, int expectArgs,
                     char delim, const char* help, const char* flag1,
    ...
    std::string overview;
    // A synopsis of command and options usage to show expected order of input
    // arguments.
    // It's the second section to get printed in the full usage message.
    std::string syntax;
    // Example (third) section in usage message.
    std::string example;
    // Final section printed in usage message. For contact, copyrights, version
    // info.
    std::string footer;
    ...
    // Maps group id to a validator index into vector of validators. Validator
    // index is -1 if there is no validator for group.
    std::map<int, int> groupValidators;
};
/* ################################################################### */
ezOptionParser::~ezOptionParser() { reset(); }
/* ################################################################### */
void ezOptionParser::reset()
{
    ...
    int i;
    for ( i = 0; i < (long int)groups.size(); ++i ) delete groups[i];
    groups.clear();

    for ( i = 0; i < (long int)unknownArgs.size(); ++i ) delete unknownArgs[i];
    unknownArgs.clear();

    for ( i = 0; i < (long int)firstArgs.size(); ++i ) delete firstArgs[i];
    firstArgs.clear();

    for ( i = 0; i < (long int)lastArgs.size(); ++i ) delete lastArgs[i];
    lastArgs.clear();
...
{
    int i;
    for ( i = 0; i < (long int)groups.size(); ++i ) groups[i]->clearArgs();
...
/* ################################################################### */
void ezOptionParser::add( const char* defaults, bool required, int expectArgs,
                          char delim, const char* help, const char* flag1,
...
void ezOptionParser::add( const char* defaults, bool required, int expectArgs,
                          char delim, const char* help, const char* flag1,
                          const char* flag2, ezOptionValidator* validator )
{
    int          id = this->groups.size();
    OptionGroup* g  = new OptionGroup;
    g->defaults     = defaults;
...
void ezOptionParser::add( const char* defaults, bool required, int expectArgs,
                          char delim, const char* help, const char* flag1,
                          const char* flag2, const char* flag3,
                          const char* flag4, ezOptionValidator* validator )
{
    int          id = this->groups.size();
    OptionGroup* g  = new OptionGroup;
    g->defaults     = defaults;
...
bool ezOptionParser::exportFile( const char* filename, bool all )
{
    ...
    if ( firstArgs.size() > 1 ) out.append( "\n" );
        ...
        if ( g->isSet || all ) {
            if ( !g->isSet || g->args.empty() ) {
                if ( !g->defaults.empty() ) {
                    ...
                    out.append( " " );
                    quote = ( ( g->defaults.find_first_of( " \t" ) !=
                                std::string::npos ) &&
                              ( g->defaults.find_first_of( "\'\"" ) ==
                                std::string::npos ) );
                    if ( quote ) out.append( "\"" );
...
bool ezOptionParser::importFile( const char* filename, char comment )
{
    ...
    for ( iter = lines.begin(); iter != lines.end(); ++iter ) {
        ...
        if ( n ) {
            ...
            if ( line->at( 0 ) == '"' ) dq.push_back( 0 );

            if ( line->at( 0 ) == '\'' ) sq.push_back( 0 );
        } else {  // Empty line.
            continue;
        }
        ...
        for ( i = 1; i < n; ++i ) {
            if ( ( line->at( i ) == comment ) &&
                 ( line->at( i - 1 ) != '\\' ) ) {
                // If within open/close quote pair, then not real comment.
                if ( sq.size() ) {
                    lo = std::lower_bound( sq.begin(), sq.end(), i );
                    // All start of strings will be even indices, closing quotes
                    // is odd indices.
                    j = (int)( lo - sq.begin() );
                    if ( ( j % 2 ) ==
                         0 ) {  // Even implies comment char not in quote pair.
    ...
    if ( argv ) free( argv );
    for ( iter = lines.begin(); iter != lines.end(); ++iter ) delete *iter;
                                           ...
                                           Layout layout )
{
    ...
    for ( i = 0; i < (long int)groups.size(); ++i ) {
        std::sort( groups[i]->flags.begin(), groups[i]->flags.end(),
                   CmpOptStringPtr );
    ...
    for ( i = 0; i < (long int)groups.size(); ++i ) {
        ...
        // stringPtrToIndexMap[stringPtrs[i]], stringPtrs[i]->c_str());
        ...
        for ( j = 0; j < groups[k]->flags.size() - 1; ++j ) {
            ...
            if ( (long int)opts.size() > width ) opts.append( "\n" );
        }
    ...
    // Number of whitespaces from start of line to description (interleave
    // layout) or
    // gap between flag names and description (align, stagger layouts).
    int gutter = 3;

    // Find longest opt flag string to set column start for help usage
    // descriptions.
    int maxlen = 0;
    if ( layout == ALIGN ) {
        for ( i = 0; i < (long int)groups.size(); ++i ) {
    ...
    for ( i = 0; i < (long int)groups.size(); ++i ) {
        ...
        if ( layout == STAGGER ) maxlen = sortedOpts[i].size();

        int pad   = gutter + maxlen;
        helpwidth = width - pad;

        // All the following split-fu could be optimized by just using substring
        // (offset, length) tuples, but just to get it done, we'll do some
        // not-too expensive string copying.
        SplitDelim( groups[k]->help, '\n', desc );
        // Split lines longer than allowable help width.
        for ( insertionIter = desc.begin(), cIter = insertionIter++;
              cIter != desc.end(); cIter = insertionIter++ ) {
            if ( (long int)( ( *cIter )->size() ) > helpwidth ) {
                ...
                while ( (long int)rem->size() > helpwidth ) {
                    ...
                    if ( rem->at( helpwidth ) == ' ' ) {
                        // If word ends exactly at helpwidth, then split after
                        // it.
                        pos = helpwidth;
                    } else {
                        // Otherwise, split occurs midword, so find whitespace
                        // before this word.
                        pos = rem->rfind( " ", helpwidth );
                    }
                    // Insert split string.
                    desc.insert( insertionIter,
                                 new std::string( *rem, 0, pos ) );
        ...
        if ( desc.size() > 0 ) {  // Crash fix by Bruce Shankle.
            // First line already padded above (before calling SplitDelim) after
            // option flag names.
            cIter = desc.begin();
            usage.append( **cIter );
            usage.append( "\n" );
    ...
    for ( i = 0; i < (long int)groups.size(); ++i ) {
        ...
        if ( g->isSet ) {
            ...
            for ( j = 0; j < (long int)g->args.size(); ++j ) {
                if ( ( g->expectArgs != -1 ) &&
                     ( g->expectArgs != (long int)g->args[j]->size() ) )
                    badOptions.push_back( *g->flags[0] );
            }
        }
    }
...
/* ################################################################### */
bool ezOptionParser::gotValid( std::vector<std::string>& badOptions,
                               std::vector<std::string>& badArgs )
{
...
void ezOptionParser::parse( int argc, const char* argv[] )
{
    ...
    */
    ...
    for ( i = 0; i < argc; ++i ) {
        ...
        if ( optionGroupIds.count( s ) ) break;
    }
            ...
            if ( g->expectArgs ) {
                if ( g->args.empty() ) {
                    ...
                    for ( k = 0; k < (long int)g->args.size(); ++k ) {
                        for ( j = 0; j < (long int)g->args[k]->size() - 1;
                              ++j ) {
                            sprintf( tmp, "%s%c", g->args[k]->at( j )->c_str(),
                                     g->delim );
                            out += tmp;
                        }
                        sprintf( tmp, "%s\n", g->args[k]->back()->c_str() );
