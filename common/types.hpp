...
struct Matrix {
    ...
    Matrix() : _b( 0 ) {}
    Matrix( int a, int b, value_type const& initial = value_type() ) : _b( 0 )
    {
        resize( a, b, initial );
    }
    Matrix( Matrix const& other ) : _data( other._data ), _b( other._b ) {}
    ...
    value_type* operator[]( int a ) { return &_data[a * _b]; }
    value_type const* operator[]( int a ) const { return &_data[a * _b]; }

    value_type& operator()( int x, int y ) { return _data[x * _b + y]; }
    ...
    {
        s << "<Matrix at " << &value << " dimensions " << value.dim_a() << 'x'
          << value.dim_b();
        if ( !value.empty() ) {
            bool                                       first = true;
...
template <>
double toEnergy<Meter>( Meter from )
{
    return 4.556335e-8 / from.value();
};

template <>
double toEnergy<eV>( eV from )
{
    return from.value() * 0.03674932;
};
}
inline constexpr units::Meter operator"" _nm( long double d )
{
    return units::Meter( d * 1e-9 );
};
inline constexpr units::Meter operator"" _cm( long double d )
{
    return units::Meter( d * 1e-2 );
};

inline constexpr units::eV operator"" _eV( long double d )
{
    return units::eV( d );
};
