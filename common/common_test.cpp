...
int main( int argc, char** argv )
{
    ...
    std::vector<float> b = common::vector_type_change<int, float>(
        c );  // This is what I want to work..
    std::cout << b[0] + .01 << ", " << b[1] + .01 << ", " << b[2] + .01
              << std::endl;

    std::cout << "import then change vector: " << std::endl;
    std::vector<float> d = common::vector_type_change<int, float>(
        common::import_vector_binary<int>(
            "fname" ) );  // This is what I want to work..
    std::cout << d[0] + .01 << ", " << d[1] + .01 << ", " << d[2] + .01
              << std::endl;
}
