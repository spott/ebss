...
{
  public:
    AbsorberParameters( int argc, const char** argv, MPI_Comm comm )
        : Parameters( comm )
    {
        register_parameters();
        ...
        if ( t == "cosine" )
            ...
            std::cerr << "absorber type " << t
                      << " not supported, defaulting to cosine" << std::endl;
            type_ = COSINE;
        }
                                 ...
                                 HamiltonianParameters<PetscReal>* hparams )
{
    ...
    if ( n_size_ == 0 && l_size_ == 0 && m_size_ == 0 ) return;
        ...
        for ( int i = start; i < end; i++ ) {
            ...
            if ( ( hparams->nmax() - prototype[i].n ) < n_size() ) {
                val = std::complex<double>(
                    ...
                    -.5 * math::PI *
                        ( 1 -
                          double( hparams->nmax() - prototype[i].n ) /
                              double( n_size() ) ) );
                std::cout << prototype[i].n << " " << hparams->nmax() << " "
                          << n_size() << ": " << val << std::endl;
                val = std::exp( val );
                VecSetValue( *abs, i, val, INSERT_VALUES );
            }
            // if ((hparams->lmax() - prototype[i].l) < l_size())
            // val *= std::exp(
            // std::complex<double>(0,((hparams->lmax() - prototype[i].l) *
            // math::PI)/(2*l_size()))
            //);
            // if ((hparams->mmax() - std::abs(prototype[i].m)) < m_size())
            // val *= std::exp(
            // std::complex<double>(0,((hparams->mmax() - prototype[i].m) *
            // math::PI)/(2*m_size()))
            //);
        }
    }
...
std::string AbsorberParameters::print() const
{
    ...
    if ( type_ == COSINE ) out << "-absorber_type cosine" << std::endl;
    if ( type_ == CX_ROT ) out << "-absorber_type cx_rot" << std::endl;
    if ( type_ == CX_SCALE ) out << "-absorber_type cx_scale" << std::endl;
    return out.str();
}
...
void AbsorberParameters::save_parameters()
{
    ...
    if ( type_ == COSINE ) file << "-absorber_type cosine" << std::endl;
    if ( type_ == CX_ROT ) file << "-absorber_type cx_rot" << std::endl;
    if ( type_ == CX_SCALE ) file << "-absorber_type cx_scale" << std::endl;
    file.close();
}
void AbsorberParameters::register_parameters()
{
    ...
    opt.add( "10", 0, 1, 0, "size of absorber in l",
             std::string( prefix ).append( "l_size\0" ).c_str() );
    opt.add( "0", 0, 1, 0, "size of absorber in m",
             std::string( prefix ).append( "m_size\0" ).c_str() );
    opt.add( ".125", 0, 1, 0, "the exponential on the cosine",
             std::string( prefix ).append( "cos_factor\0" ).c_str() );
    opt.add( "cosine", 0, 1, 0, "type (one of: cosine, cx_rot, or cx_scaled)",
