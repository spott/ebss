...
{
  public:
    HamiltonianParameters( int argc, const char** argv, MPI_Comm comm )
        : Parameters( comm )
    {
        register_parameters();
        ...
        if ( !analytic_ ) {
            ...
            try {
                basis_ = new BasisParameters<write_type_, write_type_>(
                    basis_config_, comm_ );
        ...
        // If we are bigger than the basis we are using, shrink to fit, and
        ...
        if ( !analytic_ ) {
            if ( opt.isSet( "-hamiltonian_config" ) )
                this->prototype_ = common::import_vector_binary<BasisID>(
                    this->prototype_filename() );
            else {
                this->prototype_.reserve(
                    this->basis_->basis_prototype().size() );

                if ( this->nmax() > this->basis_->nmax() ||
                     this->lmax() > this->basis_->lmax() ) {
                    if ( this->rank() == 0 ) {
                        std::cerr << output::red;
                        std::cerr << "======================ERROR=============="
                                     "=========="
                                  << std::endl;
                        std::cerr << "=The basis you are attempting to use is "
                                     "smaller   ="
                                  << std::endl;
                        std::cerr << "= than requested, your requested values "
                                     "have been ="
                                  << std::endl;
                        std::cerr << "= changed to fit                         "
                                     "         ="
                                  << std::endl;
                        std::cerr << "======================ERROR=============="
                                     "=========="
                ...
                // make the prototype... be sure to order by l's first, then j's
                // then n's.
                            ...
                            for ( int n = 1; n <= nmax_; ++n ) {
                                if ( l <= n - 1 ) {
                                    auto loc = std::find_if(
                                        start, end,
                                        [n, l, j]( const BasisID& a ) {
                                            return a.n == n && a.l == l &&
                                                   a.j == j;
                                        } );
                                    if ( loc == end )
                                        throw std::out_of_range(
                                            "didn't find the value we were "
    ...
    HamiltonianParameters( MPI_Comm comm, std::string fname )
        : Parameters( comm )
    {
...
void HamiltonianParameters<write_type_>::save_parameters()
{
    common::export_vector_binary( this->prototype_filename(),
                                  this->prototype_ );
    // opt.exportFile(std::string(folder_).append("/Hamiltonian.config\0").c_str(),
    // true);
    ...
    if ( analytic_ ) file << "-hamiltonian_analytical" << std::endl;
    file.close();
}
...
{
    common::petsc_binary_write( this->dipole_matrix_filename(), D,
                                this->comm_ );
}
...
{
    return common::petsc_binary_read<Mat>( this->dipole_matrix_filename(),
                                           this->comm_ );
}
...
void HamiltonianParameters<write_type_>::write_energy_eigenvalues()
{
    ...
    common::petsc_binary_write( this->energy_eigenvalues_filename(), ev,
                                this->comm_ );
}
...
{
    return common::petsc_binary_read<Vec>( this->energy_eigenvalues_filename(),
                                           this->comm_ );
}
...
template <typename write_type_>
std::string
HamiltonianParameters<write_type_>::energy_eigenvalues_filename() const
{
    std::stringstream ss;
    ss << folder_;
