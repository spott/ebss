...
{
  public:
    BSpline( const std::vector<scalar>* knots_, const int num_,
             const scalar scalingFactor_ = 1 )
    ...
    int left_knot( scalar x )
    {
        for ( int i = 1; i < ( *knots ).size(); i++ )
            if ( ( *knots )[i - 1] <= x && x < ( *knots )[i] ) return i - 1;
        return ( *knots ).back();
    }
    ...
    scalar derivative( scalar x, int dn )
    {
        ...
        if ( x < U[num] || x >= U[num + order + 1] ) return 0;
        ...
        deriv[0] = N[0][order];
        for ( int k = 1; k <= dn; k++ ) {
            for ( int j = 0; j <= k; j++ ) ND[j] = N[j][order - k];
    ...
    scalar operator()( scalar x )
    {
        ...
        if ( ( num == 0 && x == ( *knots )[0] ) ||
            ...
            return 1.0;
        if ( x < ( *knots )[num] || x >= ( *knots )[num + order + 1] ) return 0;
        for ( int j = 0; j <= order; j++ ) {
