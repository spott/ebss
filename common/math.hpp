...
// CBLAS_SIDE Side, OPENBLAS_CONST enum CBLAS_UPLO Uplo, OPENBLAS_CONST
// blasint M, OPENBLAS_CONST blasint N, OPENBLAS_CONST double alpha,
// OPENBLAS_CONST double *A, OPENBLAS_CONST blasint lda, OPENBLAS_CONST
// double *B, OPENBLAS_CONST blasint ldb, OPENBLAS_CONST double beta,
...
class interpolate
{
  public:
        ...
        : y_( y ), x_( x ), coefficient( x.size() )
    {
        ...
        // populate and solve the tridiagonal system, saving the solution
        // in coefficient_?
    ...
    double operator()( double x )
    {
        ...
        auto a =
            coefficient[p] * ( x_[p + 1] - x_[p] ) - ( y_[p + 1] - y_[p] );
        auto b = -coefficient[p + 1] * ( x_[p + 1] - x_[p] ) +
                 ( y_[p + 1] - y_[p] );
  ...
  private:
    ...
    size_t find_nearest_point( double x )
    {
        ...
        throw std::out_of_range(
            "couldn't find nearest point... bug in code" );
    }
...
template <typename scalar>
std::array<scalar, 3> GCCoefficients( const BasisID& init,
                                      const BasisID& fin )
{
...
void FieldFreePropagate( Vec* H, Vec* wf, PetscReal dt ) // propagate
                                                         // forward in time
{
    PetscReal norm1;
    ...
    if ( std::abs( norm1 - norm2 ) / norm1 > 10e-8 ) {
        ...
        std::cerr << "after: " << norm2 - 1
                  << " difference: " << norm1 - norm2 << std::endl;
    }
                               ...
                               Func f )
{
    ...
    if ( result != result ||
        ...
        std::cerr << "integrateTrapezoidRule: results in NaN or infinity"
                  << std::endl;
        throw( std::out_of_range(
            "integrateTrapezoidRule: results in NaN" ) );
    }
                               ...
                               const std::vector<scalar>& grid )
{
    ...
    // we assume that we only have one grid size that we ever integrate, so
    // we create the difference grid:
                      ...
                      const std::vector<scalar>& grid )
{
    ...
    // std::cerr << "trap: " << a << " simpsons: " << b << " error: " <<
    // a-b << " rel: " << a - b / a << std::endl;
    return b;
}

template <typename scalar>
scalar normalize( std::vector<scalar>& wf,
                  const std::vector<scalar>& grid )
{
    // std::function< scalar (scalar) > f = [](scalar r){ return 1.;};
    scalar norm = integrateTrapezoidRule( wf, wf, grid,
                                          []( scalar ) { return 1.; } );
    norm = std::sqrt( norm );
...
std::vector<double>
coulomb_wave_function( kBasisID a, const std::vector<double>& grid )
{
...
std::vector<double>
gsl_coulomb_wave_function( const kBasisID& a,
                           const std::vector<double>& grid )
{
    ...
    // std::cerr << std::endl << "k: " << a.k << ", l:" << a.l <<
    // std::endl;

    for ( auto& r : grid ) {
        double exp;
        // we should really calculate more than just the one l, but not
        // right now.
        gsl_sf_coulomb_wave_F_array( a.l, 0, -1. / a.k, a.k * r, val,
                                     &exp );
        if ( *val != *val ||
...
inline std::complex<double> Gamma_Lanczos( std::complex<double> z )
{
    ...
    if ( real( z ) < 0.5 ) // reflection
        ...
        return std::sqrt( 2. * PI ) * std::pow( t, z + 0.5 ) *
               std::exp( -t ) * x;
    }
}
...
// std::vector< std::tuple< PetscScalar, int > > VecSort(Vec a, Func
// comparator)
//{
////mergesort
...
std::vector<std::tuple<PetscScalar, int>>
VecQuickSelect( Vec a, size_t n, Comparator comp )
{
}
...
std::vector<std::tuple<PetscScalar, int>>
VecStupidSelect( Vec a, Selector select )
{
    Vec local;
    VecScatter scatter;
    VecScatterCreateToAll( a, &scatter, &local );
...
template <typename Comparator>
std::vector<std::tuple<PetscScalar, int>>
VecFirstNSort( Vec a, size_t n, Comparator comp )
{
    std::vector<PetscInt> outint( n, -1 );
    std::vector<PetscScalar> outscalar( n,
    ...
    if ( rank == 0 ) {
        ...
        std::copy( outscalar.begin(), outscalar.end(),
                   allscalars.begin() );
    ...
    for ( size_t i = 0; i < n; ++i ) {
        ...
        out.push_back(
            std::make_tuple( outscalar.at( i ), outint.at( i ) ) );
    }
...
fourier( std::vector<double>&& time_series )
{
