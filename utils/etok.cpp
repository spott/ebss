...
int main( int argc, const char** argv )
{
    ...
    std::function<bool( int, int )> test = [&hp, &kp,
                                            &kparams]( int i, int j ) -> bool {
        return ( hp[j].l == kp[i].l && hp[j].n < kparams.nmax() &&
                 hp[j].e.real() > 0.0 );
    };

    std::function<std::shared_ptr<std::vector<double>>( BasisID )> import_wf =
        [&kparams]( BasisID a ) -> std::shared_ptr<std::vector<double>> {
        std::shared_ptr<std::vector<double>> b( new std::vector<double>(
            std::move( common::import_vector_binary<double>(
                kparams.hamiltonian()
                    .basis_parameters()
                    ->basis_function_filename( a ) ) ) ) );
        return b;
    };
    ...
    std::function<std::shared_ptr<std::vector<double>>( kBasisID )> coulombf =
        [&kparams](
            const kBasisID& a ) -> std::shared_ptr<std::vector<double>> {
        try {
            std::shared_ptr<std::vector<double>> b(
                new std::vector<double>( std::move(
                    math::gsl_coulomb_wave_function( a,
                                                     *( kparams.hamiltonian()
                                                            .basis_parameters()
                                                            ->grid() ) ) ) ) );
            return b;
            ...
            b( new std::vector<double>( std::move( math::coulomb_wave_function(
                a,
                *( kparams.hamiltonian().basis_parameters()->grid() ) ) ) ) );
            return b;
        }
    ...
    std::function<std::complex<double>( int, int )> value =
        [&hp, &kp, &kparams, &import_wf, &coulombf]( int i, int j ) {
            auto wf  = import_wf( hp[j] );
            auto cwf = coulombf( kp[i] );
            return math::integrateTrapezoidRule(
                *wf, *cwf,
                *( kparams.hamiltonian().basis_parameters()->grid() ),
                []( double r ) -> double { return 1.; } );
        };


    Mat etok = common::populate_matrix<std::complex<double>>(
        kparams, test, value, kp.size(), hp.size(), false );
