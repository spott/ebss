...
{
    // takes six arguments: the min, the max, the dtau the base folder filename,
    ...
    // std::cout << min << " " << max << " " << dtau << " " << rep_name <<
    // std::endl;
    ...
    for ( size_t i = 0; i < ( max - min ) / dtau; ++i ) {
        sprintf( buffer, "%s%.1f%s/after.dat", first.c_str(), i * dtau,
                 end.c_str() );
    ...
    for ( auto i = file_names.begin(); i < file_names.end(); ++i ) {
        ...
        if ( skip * dt - dtau > 1e-10 )
            std::cerr << "there isn't a good correlation between dtau and dt"
                      << std::endl;
        std::cout << dt << " " << skip << std::endl;

        for ( auto i = v.begin(); i - v.begin() < ( skip * N );
              i += skip, ++current_place )
    ...
    std::vector<std::complex<double>> output(
        ...
        reinterpret_cast<std::complex<double>*>( out + current_place ) );
