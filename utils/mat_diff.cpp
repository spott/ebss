...
int main( int argc, char** argv )
{
    ...
    if ( argc != 4 && argc != 3)
    {
        std::cout << "two or three arguments expected: filename for matrix 1, matrix 2, and matrix 2 - matrix 1 " << std::endl;
    ...
    PetscViewer view;
    PetscViewerBinaryOpen(PETSC_COMM_WORLD, fnames[0].c_str(),FILE_MODE_READ,&view);
    MatLoad(mat1, view);
    PetscViewerBinaryOpen(PETSC_COMM_WORLD, fnames[1].c_str(),FILE_MODE_READ,&view);
    MatLoad(mat2, view);
    ...
    auto proto = common::import_vector_binary<BasisID>("500/120000/300_hamiltonian/vector_prototype.dat");
    ...
    {
        std::cout << proto[i] << " <--> " << proto[idx[i]] << " = " << p[i] << std::endl;
    }
    //for(auto a: idx)
    //{
        //std::cout << a << std::endl;
    ...
    PetscViewerBinaryOpen(PETSC_COMM_WORLD, fnames[2].c_str(),FILE_MODE_WRITE,&view);
    MatView(mat1, view);
