...
int main( int argc, const char** argv )
{
    ...
    HamiltonianParameters<PetscReal>* params =
        new HamiltonianParameters<PetscReal>( MPI_COMM_WORLD,
                                              std::string( bagname ) );
    PulsetrainParameters* lparams =
        new PulsetrainParameters( argc, argv, MPI_COMM_WORLD );
    AbsorberParameters* aparams =
        new AbsorberParameters( argc, argv, MPI_COMM_WORLD );
    StateParameters* sparams =
        new StateParameters( argc, argv, MPI_COMM_WORLD );
    DipoleParameters* dparams =
        new DipoleParameters( argc, argv, MPI_COMM_WORLD );
    ...
    if ( params->rank() == 0 ) {
        ...
        common::export_vector_ascii( std::string( "./prototype.csv" ),
                                     params->prototype() );
