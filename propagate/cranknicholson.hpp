...
PetscErrorCode solve( Vec* wf, context* cntx, Mat* A )
{
    ...
    while ( t < maxtime ) {
        ...
        // MatAXPY(*A, cn_factor * (cntx->laser->efield(t+cntx->laser->dt()) -
        // ef), *( cntx->D ), SAME_NONZERO_PATTERN);
        MatScale( *A, std::complex<double>( -1, 0 ) ); // A = i * .5 dt
                                                       // [ef(t+dt) * D + H_0 ]
                                                       // - 1
        MatShift( *A, std::complex<double>( 2, 0 ) ); // A = i * .5 dt [ef(t+dt)
                                                      // * D + H_0 ] + 1

        // MatScale(*A, ef );                                   // A = ef(t) * D
        // MatDiagonalSet(*A, *(cntx->H), INSERT_VALUES);      // A = ef(t) * D
        // + H_0
        // MatScale(*A, cn_factor);                            // A = - i * .5 *
        // dt [ ef(t) * D + H_0 ]
        // MatShift(*A, std::complex<double>(1,0));            // A = - i * .5 *
        // dt [ ef(t) * D + H_0 ] + 1
        // MatMult(*A, *wf, tmp);                              // A u_n = tmp
        // MatAXPY(*A, cn_factor * (eff - ef), *( cntx->D ),
        // SAME_NONZERO_PATTERN);
        // MatScale(*A, std::complex<double>(-1,0));           // A = i * .5 dt
        // [ef(t+dt) * D + H_0 ] - 1
        // MatShift(*A, std::complex<double>(2,0));            // A = i * .5 dt
        // [ef(t+dt) * D + H_0 ] + 1

        KSPSetOperators(
            ksp, *A, *A, SAME_NONZERO_PATTERN ); // Solve[ A x = tmp ] for x
        ...
        //{
        // if (cntx->hparams->rank() == 0)
        // std::cout << "time: " << t << " step: " << step << " efield: " << ef
        // << " norm-1: " << norm-1 << " * " << p << "/4" << std::endl;
        // std::ostringstream wf_name;
        ...
        // p++;
        //}
        // if (( ef.real() - cntx->laser->efield(t - cntx->laser->dt()).real() >
        // 0 && cntx->laser->efield(t + cntx->laser->dt()).real() 0 ef.real() <
        // 0) || ( ef.real() - cntx->laser->efield(t - cntx->laser->dt()).real()
        // < 0 && cntx->laser->efield(t + cntx->laser->dt()).real() 0 ef.real()
        // > 0)) //maximum of the field
        //{
        // if (cntx->hparams->rank() == 0)
        // std::cout << "time: " << t << " step: " << step << " efield: " << ef
        // << " norm-1: " << norm-1 << " * " << p << " max " << std::endl;
        // std::ostringstream wf_name;
    ...
    // After the propagation through diffeq, we need to do the propagation after
    // the fact to find the dipole
    // moment over time.
    ...
    //while ( ( t - maxtime ) <= cntx->dipole->t_after() ) {
        ...
        //after_dipole.push_back(
            //cntx->dipole->find_dipole_moment( *( cntx->D ), tmp ) );
        //t += cntx->dipole->dt();
    //}
    ...
    //if ( cntx->hparams->rank() == 0 ) {
        //try
        //{
            //common::export_vector_binary( cntx->dipole->after_filename(),
                                          //after_dipole );
        //}
        //catch ( ... )
        //{
            //std::cerr << "couldn't open the after_dipole file, oops... it "
                         //"won't be written to disk... NOOO!!" << std::endl;
        //}
        ...
        //catch ( ... )
        //{
            //std::cerr << "couldn't open the after_time file, oops... it won't "
                         //"be written to disk... NOOO!!" << std::endl;
        //}
    //}
    ...
    //if ( cntx->hparams->rank() == 0 ) {
        ...
        //catch ( ... )
        //{
            //std::cerr << "couldn't write out the dipole_fourier files.. :( "
                      //<< std::endl;
        //}
    //}
