...
void display_function( Gnuplot g, std::vector<scalar> grid,
                       std::vector<scalar> wf, int point = -1,
                       bool wait = false, std::array<int, 2> range = {0, 20} )
{
    g.reset_all();
    ...
    if ( point > 0 && point < grid.size() )
        g.set_style( "points" )
            .plot_xy( std::vector<double>{grid[point]},
                      std::vector<double>{wf[point]}, "turnover" );
