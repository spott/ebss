...
namespace numerov
{
    ...
    template <typename scalar>
    std::ostream& operator<<(std::ostream &out, const it<scalar> &b)     //output using << operator
    {
    ...
    template <typename scalar>
        ...
        find_basis( const BasisID state, 
                    ...
                    const std::function< scalar (scalar) > pot, 
                    const scalar err)
        {

            std::array< scalar, 3 > fore;  //I don't need a memory of these...
            ...
            current.energy_upper = pot(rgrid[ wf.size()-1 ]);
            ...
            for (size_t i = 0; i < f.size(); i++)
            {
                current.energy_lower = std::min (
                        ...
                        std::pow( scalar(state.l) + .5, 2) / (2. * std::pow(rgrid[i],2) ) + pot(rgrid[i])
                        );
            }
            ...
            current.energy_upper = 2;
            current.de = 1e-3; //we don't want to converge too quickly
            ...
            while (!converged)
            {
                ...
                f[0] = 1 + dx * dx / 12 * ( - std::pow((static_cast<scalar>(state.l) + .5), 2)
                        - 2 * std::pow(rgrid[0],2) * (pot(rgrid[0]) - current.energy));
                ...
                for (size_t i = 1; i < f.size(); i++)
                {
                    f[i] = dx * dx / 12 * ( - std::pow((static_cast<scalar>(state.l) + .5), 2)
                            - 2 * std::pow(rgrid[i],2) * (pot(rgrid[i]) - current.energy));
    ...
    template <typename scalar, typename write_type>
        void find_basis_set( std::function< scalar (scalar) > pot, 
                             BasisParameters<scalar, write_type> *params, 
                             scalar gs)
        {
            //Make logarithmic grid:
            ...
            for (int l = 0; l <= params->lmax(); l ++)
            {
                ...
                else //or the l == 0 state energy we have already found
                {
                    tmp = *std::find_if(
                            energies->begin(), 
                            energies->end(), 
                            [l](const BasisID &a) {return (a.n == l+1 && a.l == l-1); }
                            );
                    tmp.e = tmp.e*1.1;
                }
                for (int n = l+1; n <= params->nmax(); n++)
                {
                    ...
                    std::cerr << n << "\t" << l << "\t" << res.energy << std::endl;
                    std::cerr << "=========================================" << std::endl;
                    std::cerr << "=========================================" << std::endl;
                    std::cerr << "=========================================" << std::endl;
                    std::cerr << "=========================================" << std::endl;
