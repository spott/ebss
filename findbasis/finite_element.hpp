...
PetscReal find_value( const parameters                              params,
                      const boost::function<PetscReal( PetscReal )> potential,
                      const std::vector<PetscReal> knots, const int i,
                      const int j, const PetscReal dr )
{
    // Here I'm using BSplines, but theoretically, I could use any
    // finitely bound function.
    element<order, scalar> bsi( &knots, i );
    element<order, scalar> bsj( &knots, j );
                 ...
                 const boost::function<scalar( scalar )> potential )
{
    ...
    PetscReal dr =
        ( params.rmax() - params.rmin() ) / ( params.points() - order * 2 );

    for ( it = knots.begin(); it < knots.end(); it++ ) {
        if ( it - knots.begin() <= order ) *it = params.rmin();
        if ( knots.end() - it <= order ) *it   = params.rmax();
        *it = params.rmin() + ( it - knots.begin() ) * dr;
    }
    boost::function<bool( int, int )>   t  = boost::cref( test<order> );
    boost::function<scalar( int, int )> fv = boost::bind<PetscReal>(
        find_value<order, scalar, element>, boost::ref( params ),
        boost::cref( potential ), boost::cref( knots ), _1, _2, dr );
    Mat H =
        common::populate_matrix( params, t, fv, params.points() - order * 2 );
