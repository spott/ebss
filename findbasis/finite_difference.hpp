...
std::vector<BasisID>
find_basis( std::function<scalar( scalar, BasisID )> pot,
            ...
            const BasisID& state )
{
    ...
    Mat H =
        common::populate_matrix( static_cast<const Parameters>( params ),
                                 t,
                                 fv,
                                 params.points(),
                                 params.points(),
    ...
    EPSSetDimensions(
        eps, params.nmax() - state.l, PETSC_DECIDE, PETSC_DECIDE );
    ...
    PetscPrintf(
        params.comm(), " Number of iterations of the method: %D\n", its );
    EPSGetDimensions( eps, &nev, PETSC_NULL, PETSC_NULL );
    PetscPrintf(
        params.comm(), " Number of requested eigenvalues: %D\n", nev );
    ...
    PetscPrintf(
        params.comm(), " Number of converged eigenpairs: %D\n\n", nconv );
...
template <typename scalar, write_type>
